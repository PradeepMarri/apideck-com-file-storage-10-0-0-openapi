...
func FoldersupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.UpdateFolderRequest
		...
		url := fmt.Sprintf("%s/file-storage/folders/%s%s", cfg.BaseURL, id, queryString)
		...
		var result models.UpdateFolderResponse
...
func CreateFoldersupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_file-storage_folders_id",
		mcp.WithDescription("Rename or move Folder"),
		...
		mcp.WithString("name", mcp.Description("Input parameter: The name of the folder.")),
		mcp.WithString("parent_folder_id", mcp.Description("Input parameter: The parent folder to create the new file within.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    FoldersupdateHandler(cfg),
	}
}
