...
func FoldersaddHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateFolderRequest
		...
		url := fmt.Sprintf("%s/file-storage/folders%s", cfg.BaseURL, queryString)
		...
		var result models.CreateFolderResponse
...
func CreateFoldersaddTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_file-storage_folders",
		mcp.WithDescription("Create Folder"),
		...
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: The name of the folder.")),
		mcp.WithString("parent_folder_id", mcp.Required(), mcp.Description("Input parameter: The parent folder to create the new file within.")),
		mcp.WithString("description", mcp.Description("Input parameter: Optional description of the folder.")),
		mcp.WithString("drive_id", mcp.Description("Input parameter: ID of the drive to create the folder in.")),
		mcp.WithString("id", mcp.Description("Input parameter: A unique identifier for an object.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    FoldersaddHandler(cfg),
	}
}
