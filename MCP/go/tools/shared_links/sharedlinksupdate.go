...
func SharedlinksupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.SharedLink
		...
		url := fmt.Sprintf("%s/file-storage/shared-links/%s%s", cfg.BaseURL, id, queryString)
		...
		var result models.UpdateSharedLinkResponse
...
func CreateSharedlinksupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_file-storage_shared-links_id",
		mcp.WithDescription("Update Shared Link"),
		...
		mcp.WithString("target_id", mcp.Required(), mcp.Description("Input parameter: The ID of the file or folder to link.")),
		mcp.WithString("expires_at", mcp.Description("")),
		mcp.WithString("scope", mcp.Description("Input parameter: The scope of the shared link.")),
		mcp.WithString("created_at", mcp.Description("Input parameter: The date and time when the object was created.")),
		mcp.WithBoolean("password_protected", mcp.Description("Input parameter: Indicated if the shared link is password protected.")),
		mcp.WithString("download_url", mcp.Description("Input parameter: The URL that can be used to download the file.")),
		...
		mcp.WithString("url", mcp.Description("Input parameter: The URL that can be used to view the file.")),
		mcp.WithString("password", mcp.Description("Input parameter: Optional password for the shared link.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    SharedlinksupdateHandler(cfg),
	}
}
