...
func UploadsessionsuploadHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["part_number"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("part_number=%v", val))
		}
		...
		url := fmt.Sprintf("%s/file-storage/upload-sessions/%s%s", cfg.BaseURL, id, queryString)
		...
		if val, ok := args["digest"]; ok {
			req.Header.Set("digest", fmt.Sprintf("%v", val))
		}
		...
		var result models.UpdateUploadSessionResponse
...
func CreateUploadsessionsuploadTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_file-storage_upload-sessions_id",
		mcp.WithDescription("Upload part of File to Upload Session"),
		...
		mcp.WithString("part_number", mcp.Required(), mcp.Description("Part number of the file part being uploaded.")),
		mcp.WithString("digest", mcp.Description("The RFC3230 message digest of the uploaded part. Only required for the Box connector. More information on the Box API docs [here](https://developer.box.com/reference/put-files-upload-sessions-id/#param-digest)")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UploadsessionsuploadHandler(cfg),
	}
}
