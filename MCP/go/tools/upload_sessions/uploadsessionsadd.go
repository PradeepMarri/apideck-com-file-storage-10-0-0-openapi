...
func UploadsessionsaddHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/file-storage/upload-sessions%s", cfg.BaseURL, queryString)
		...
		var result models.CreateUploadSessionResponse
...
func CreateUploadsessionsaddTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_file-storage_upload-sessions",
		mcp.WithDescription("Start Upload Session"),
		...
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: The name of the file.")),
		...
		mcp.WithNumber("size", mcp.Required(), mcp.Description("Input parameter: The size of the file in bytes")),
		mcp.WithString("drive_id", mcp.Description("Input parameter: ID of the drive to upload to.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UploadsessionsaddHandler(cfg),
	}
}
