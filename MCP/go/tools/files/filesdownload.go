...
func FilesdownloadHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/file-storage/files/%s/download%s", cfg.BaseURL, id, queryString)
...
func CreateFilesdownloadTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_file-storage_files_id_download",
		mcp.WithDescription("Download File"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    FilesdownloadHandler(cfg),
	}
}
