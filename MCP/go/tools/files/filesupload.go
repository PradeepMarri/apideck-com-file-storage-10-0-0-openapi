...
func FilesuploadHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/file-storage/files%s", cfg.BaseURL, queryString)
		...
		if val, ok := args["x-apideck-metadata"]; ok {
			req.Header.Set("x-apideck-metadata", fmt.Sprintf("%v", val))
		}
		...
		var result models.CreateFileResponse
...
func CreateFilesuploadTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_file-storage_files",
		mcp.WithDescription("Upload file"),
		mcp.WithBoolean("raw", mcp.Description("Include raw response. Mostly used for debugging purposes")),
		mcp.WithString("x-apideck-metadata", mcp.Required(), mcp.Description("Metadata to attach to the file")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    FilesuploadHandler(cfg),
	}
}
