...
func FilesexportHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/file-storage/files/%s/export%s", cfg.BaseURL, id, queryString)
...
func CreateFilesexportTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_file-storage_files_id_export",
		mcp.WithDescription("Export File"),
		...
		mcp.WithString("format", mcp.Required(), mcp.Description("File format to export this file to. A list of available file formats for the current file is available as `export_formats` on the File resource.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    FilesexportHandler(cfg),
	}
}
